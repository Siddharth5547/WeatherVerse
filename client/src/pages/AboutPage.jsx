...
export default function AboutPage() {
  ...
  return (
    <div className="space-y-12 pb-16 pt-2 max-w-5xl mx-auto">
      {/* Hero Editorial Card */}
      <div className="apple-card p-8 sm:p-14 text-center sm:text-left relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] dark:text-[#8E8AFF] border border-[var(--accent-primary)]/20 mb-4">
          ...
          <p className="text-base sm:text-lg text-[var(--text-secondary)] mt-4 leading-relaxed font-normal">
          ...
          <div className="flex flex-wrap gap-3 mt-8">
            ...
            <Link
              to="/ai"
              className="apple-btn-secondary px-6 py-2.5 text-sm font-medium"
            >
              Consult AI Copilot
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {coreCapabilities.map((cap, i) => {
            ...
            return (
              <div
                key={i}
                className="apple-card p-6 flex flex-col justify-between group hover:border-[var(--accent-primary)]/40 transition-all"
              >
                <div>
                  <div className="w-11 h-11 rounded-2xl bg-[var(--accent-primary)]/10 border border-[var(--accent-primary)]/20 flex items-center justify-center text-[var(--accent-primary)] dark:text-[#8E8AFF] mb-4 group-hover:scale-105 transition-transform">
                    <Icon size={20} />
                  </div>
                  <h3 className="font-semibold text-base text-[var(--text-primary)] mb-2">{cap.title}</h3>
      ...
      <div className="apple-card p-8">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[var(--border-subtle)]">
          <div className="w-10 h-10 rounded-2xl bg-[var(--accent-primary)]/10 border border-[var(--accent-primary)]/20 flex items-center justify-center text-[var(--accent-primary)] dark:text-[#8E8AFF]">
            <Cpu size={18} />
          </div>
          <div>
            <h3 className="font-semibold text-lg text-[var(--text-primary)]">Technology Ecosystem</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {techStack.map((tech, idx) => (
            <div
              key={idx}
              className="p-4 rounded-2xl bg-[var(--surface-card-secondary)] border border-[var(--border-subtle)]"
            >
              <span className="font-semibold text-sm text-[var(--accent-primary)] dark:text-[#8E8AFF] block">
                {tech.name}
              </span>
